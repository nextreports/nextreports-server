...
import ro.nextreports.server.web.core.search.SearchEntityPanel;

/**
 * @author Decebal Suiu
 */
public class SearchEntityActionLink extends ActionAjaxLink {

	public SearchEntityActionLink(ActionContext actionContext) {
		super(actionContext);
	}

    public void executeAction(AjaxRequestTarget target) {
        Entity entity = getActionContext().getEntity();        
        //setResponsePage(new SearchEntityPage(entity.getPath()));
        EntityBrowserPanel panel = findParent(EntityBrowserPanel.class);
        if (getActionContext().isMenuAction()) {
            NextServerSession.get().setSearchContext(null);
            panel.restoreWorkspace(target);
        }
        panel.forwardWorkspace(new SearchEntityPanel("work", entity.getPath()), target);
    }
