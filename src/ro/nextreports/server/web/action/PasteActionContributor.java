...
//
public class PasteActionContributor extends SingleActionContributor {
	
	public static final String ID = PasteActionContributor.class.getName();
    ...
    public boolean support(Entity entity) {
        if ((entity != null) && (entity instanceof Folder)) {
            PasteContext pasteContext = NextServerSession.get().getPasteContext();
            if ((pasteContext == null) ||
                 !sectionManager.getSelectedSection().getId().equals(pasteContext.getSectionId())) {
                return false;
            } else {
                String destPath = entity.getPath();
                for (String sourcePath : pasteContext.getSourcePaths()) {
    ...
    public String getActionImage() {
        return "images/paste.gif";
    }

    public String getActionName() {
    	return new StringResourceModel("ActionContributor.Paste.name", null).getString();
    }
    ...
    public AbstractLink getLink(ActionContext context) {
        return new PasteActionLink(context);
    }
