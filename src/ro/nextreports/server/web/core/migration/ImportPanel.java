...
public class ImportPanel extends Panel {
	...
	// keep a map of newly added entities paths (path, entity) (these are the
	// path before modification if any)
		...
		importForm.add(new AjaxSubmitLink("import") {
			...
			protected void onSubmit(AjaxRequestTarget target, Form<?> form) {
				...
				try {
					...
					FileUtil.backupRepository(new File(home), new File(backupPath + File.separator + "data"),
	...
	private void importSchedulers() {
		for (SchedulerJob sch : mo.getSchedulers()) {
			importEntity(sch);
		}
	}

	private void importGroups() {
		for (Group sch : mo.getGroups()) {
			importEntity(sch);
		}
	}

	private void importUsers() {
		for (User sch : mo.getUsers()) {
			importEntity(sch);
		}
	}

	private void importUserWidgetSettings() {
		for (UserWidgetParameters sch : mo.getUserWidgetParameters()) {
			importEntity(sch);
		}
	}

	// drill down relatinons must be set after all reports and charts were
	// created!
	...
	private void importCharts() throws Exception {
		for (Chart chart : mo.getCharts()) {
			...
			// import chart : must modify chart id for widget states that
			// contain the chart
	...
	private void importReports() throws Exception {
		for (Report report : mo.getReports()) {
			...
			// import report : must modify report id for widget states that
			// contain the report
	...
	private Entity importEntity(Entity importedEntity) {
		...
		try {
			...
			if (entity != null) {
				...
				// but for DashboardState we must take care for all internal
				// WidgetStates
				clearCache(entity);

				LOG.info("\tentity was updated");
			} else {
				LOG.info("\tnew entity original id {" + importedEntity.getId() + "}");
	...
	private void clearCache(Entity oldEntity) {
		if (oldEntity instanceof DashboardState) {
			// when an entire dashboard state is updated, widget states are
			// recreated with other ids
			// so we have to clear cache for children
			// widget ids are modified -> iframes for those widgets will not
			// work anymore!
